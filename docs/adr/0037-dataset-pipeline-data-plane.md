...
- **Mechanics resolved by ADR-0040:** the Dataset storage mechanism (materialised, versioned JSONB rows — same physical shape as `object_instances`), the transform execution model (a transform *is* a SQL-over-JSONB query writing a new Dataset version, reusing a table-agnostic query compiler), the write-path consolidation (`ImportEngine` subsumed — one path to `object_instances`), and the two-legs reconvergence (MVP transforms key-preserving by construction; key-preservation invariant named for grain-changers). The transform-step *catalogue contents* and incremental re-execution remain open.
