...
- **Scheduling:** the write-authz fix is deliberately **deferred to pre-launch** (not now). The deferral holds only while no untrusted principal can reach the deployment — it is sound in the OPC's own hands or a trusted internal/demo environment. The real trigger is *not* the calendar but **the first untrusted principal**, whichever comes first: (a) an SMB end-user account is issued (even a read-only consume role), or (b) the deployment becomes network-reachable outside the trusted set. Either crossing voids the deferral and makes the fix blocking before any further rollout — and may fire earlier than the formal launch date.
