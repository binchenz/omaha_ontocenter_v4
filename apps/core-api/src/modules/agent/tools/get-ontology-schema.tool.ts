...
export class GetOntologySchemaTool implements AgentTool {
  ...
  description = '获取当前租户的本体 schema（对象类型、属性、关系）。系统提示里已列出所有类型名称作为路由菜单；当你已选定某个类型、需要它的完整字段/单位/枚举细节时，传入 typeName 只取该类型的详情；不传则返回全部类型。';
  parameters = {
    type: 'object',
    properties: {
      typeName: {
        type: 'string',
        description: '只返回该对象类型的完整详情（字段、单位、枚举、关系）。用于已从系统提示的类型菜单中选定类型后按需拉取细节，避免拉取全量。需要全部类型时传空字符串 ""。',
      },
    },
    required: ['typeName'],
