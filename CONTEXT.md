...
**Agent**:
The conversational AI that is the platform's primary user interface. A single LLM-driven loop — one Agent across all Surfaces (ADR-0039), not one per Surface — that understands user intent, activates Skills, calls Tools via the SDK, and streams responses. Operates within one Tenant's Ontology. See `docs/adr/0008-agent-first-architecture.md`. **In practice it is a runtime *analysis* Agent** (ADR-0063): the only live conversational entry point (`/chat`) hardcodes the `consume` Surface, so the Agent's real, exercised job is the read-only triad **query / aggregate-analyse / chart**. ADR-0008's "Agent does *all* operations (ingest, model, act) through chat" and ADR-0039's cross-surface continuity are **scoped down** — design-time work (ingestion, modeling) is delivered via `scripts/` (IngestRecipe) against the services, not via conversation; the conversational ingestion/modeling Skills exist but are a dormant, unbuilt path. The single-Agent thesis holds for runtime; it is explicitly *not* the delivery path.
_Avoid_: Bot, Assistant, Copilot

**Skill**:
A domain capability package the Agent loads for a turn. Contains a system prompt fragment, a subset of available Tools, and optional workflow guidance. Code-defined, not tenant-configurable. Examples: data ingestion skill, ontology design skill, query skill. **Assembled per request from `{permissions, surface}`** (ADR-0039/0041): the declared Surface narrows which Skills load; absent a Surface the all-active union holds (ADR-0010). This assembly is a relevance + least-privilege layer, **not** the security gate — a wrongly-scoped Skill is a UX/accuracy regression, not a vulnerability; the write-authz gate (ADR-0040) is the real boundary. When a Skill is withheld for lack of permission, the Agent is given opening guidance to say so up front rather than fail late. A Skill must be **self-contained** in its tool references (its `tools[]` names a tool that some Skill carries) *and* every name it declares must be registered in `AGENT_TOOLS` — both directions are checked at boot (`findOrphanedTools` + `findDanglingToolRefs`, ADR-0063), because a dangling reference silently drops to a no-op under the orchestrator's `(registered ∩ declared)` scoping (this is how `render_chart` stayed un-callable while the chart panel sat blank). The `query` Skill is **self-sufficient for the runtime analysis triad**: it owns `query_objects` / `aggregate_objects` / `get_ontology_schema` / `render_chart`, so a `consume` turn can query, aggregate, and chart without loading the heavier `research_qa` Skill (ADR-0063).
...
**Vertical**:
An industry-specific capability bundle, contributed to the core as a pluggable unit — the seam that lets the platform be open-core (ADR-0062). One `Vertical` is a thin manifest fanning its contributions into the core's existing collection seams: **Tools**, **Skills**, connectors (+ pipeline provisioner), and **drill-gates**. The load-bearing rule: **core depends on zero Verticals; every Vertical depends on core** — core code carries **no named reference to any concrete Vertical**, so a community OPC adds one by writing a package against the published `Vertical` API, never by editing core. The public repo is the single source of truth and ships a neutral **reference vertical** (generic **Sales Records**, #207) so the seam has a second implementation proving it is real ("one adapter = hypothetical seam; two = real seam"). The **AVC** vertical is the private second implementation, delivered to 纯米 as core + a private `@omaha/vertical-avc` package. `Vertical` and its dependency types are a versioned **public extension API**.
_Avoid_: Plugin, Extension, Module (in agent context — reserve for the contribution-unit sense), Domain pack

**Reference Vertical** (Sales Records, #207):
The neutral two-star vertical shipped in the public repo as the community template — every ADR-0061 semantic + the **Drill-Gate** seam demoed once, structurally isomorphic to AVC but client-free. A summary star **`sales_record`** (long-format: `region` / `product` / `period` / `metric` ∈ `{units_sold, revenue, avg_price}` / `value`) demos *additivity* (units_sold/revenue `additive`, avg_price `ratio`), `collapsedDefault` (`region` defaults to 全国), and `universe: whole-market`; a detail star **`sales_line`** (per-SKU sample: `sku` / `unitPrice` / `unitsSold`) demos `universe: top-sample` and is the **drill target** (broad `sales_record` → confirm → drill `sales_line`). A community OPC copies this package, renames `sales_*` to its own domain, and has a working vertical. AVC is the same pattern "weighted up" (three stars, Excel cross-tab extractor).
_Avoid_: Demo data / seed (it is real shipped code, not a seed), Example app, Sample vertical

**Drill-Gate**:
A platform capability (generalized from the AVC-specific `AVC_DRILL_GATE`, #195/ADR-0062): "query the cheap broad layer first, then pause for user confirmation before the first drill into an expensive detail layer." The mechanism lives in the orchestrator, which consumes an injected set of `DrillGate` configs (`{broadLayer, drillTarget, confirmMessage}`) and knows no concrete object-type names. A **Vertical** contributes its drill-gates **statically** — the broad-vs-drill layering is a fixed property of the vertical's star schema, not a tenant preference (contrast `selfBrands`, which *is* per-tenant because it is customer identity, not schema shape).
_Avoid_: Confirmation gate (that is the generic suspend/resume `ConfirmationGate` service; a Drill-Gate is one policy that triggers it)
...
### Market Intelligence (AVC vertical, ADR-0042)

> **Open-core boundary (ADR-0062):** this whole section describes the **AVC Vertical**, which is a *private* package (`@omaha/vertical-avc`), not part of the open-source core. AVC reports are produced exclusively for one client, so the vertical's presence is itself client-identifying — it is therefore kept out of the public repo, which instead ships the neutral reference vertical (generic market-metric). The terms below are the AVC vertical's own domain glossary; they do not appear in core. Customer identity (the 小米/米家 self-brand merge) lives in `Tenant.settings.selfBrands`, never in code.

The domain language for the AVC market-research vertical. Two assets are joined on a declared **品类/价格段 spine** (category / price-band), with **no NER** — the spine is confirmed, never guessed. The ontology here is **decision-first**: its object grain is chosen to answer a real decision chain (trend → share-decline → price-band attribution → competitor-new-product), not to mirror the source spreadsheet tab-by-tab.
