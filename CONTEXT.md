...
**PipelineStep**:
One stage in a Pipeline's transform DAG. Three enum-constrained types (ADR-0053): `filter` (single condition, compose via multiple steps), `rename` (column renaming), `compute` (predefined functions: `normalize_brand`, `price_band`). Config is JSON Schema-validated at Pipeline creation to prevent runtime failures. Compute steps may reference TransformConfig via `configRef` + locked `configVersion`, or inline params directly.
